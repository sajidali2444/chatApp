...
const ChatBoxContainer = () => {
    ...
    const handleGreetingMessages = async (guestUser, setLoading) =>{
        debugger
        setChatText('');
       await  axios.get(`http://162.244.80.91:10870/SilviaServer/Core/GetAll?user=${guestUser}`, {headers})
            .then((resp) => {
    ...
    const handleUserNameToken = async () => {
        ...
        await axios.get(`http://162.244.80.91:10870/SilviaServer/Core/Create?user=${guestUser}&file=SilviaServerChat.sil`, {headers})
            .then((resp) => {
                    debugger
                    console.log(resp)
                if(resp?.data?.success === true){
    ...
    const handleCloseChat = async  () => {
        ...
        debugger
        if(checkLocalStorage){
         await axios.get(`http://162.244.80.91:10870/SilviaServer/Core/Release?user=${checkLocalStorage}`)
              .then((resp) => {
                  debugger
                  console.log(resp);
    ...
    const handleMessages = async () => {
        ...
        setLoading(true);
        debugger
        await axios.get(`http://162.244.80.91:10870/SilviaServer/Core/SetInput?user=${userNameToken}&text=${chatText}`, {headers})
            .then((resp) => {
                    debugger
                setUserGreetMessages((prevState) => {
